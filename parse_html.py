...
def extract_content_from_page(file_path):
    ...
    if blog_content:
        date = blog_content.find("div", class_="dt")
        title = blog_content.find("h3")
        content_paragraphs = blog_content.find_all("p")

        if date:
            parsed_data["date"] = date.text.strip()
        else:
            parsed_data["date"] = None

        if title:
            parsed_data["title"] = title.text.strip()
        else:
            parsed_data["title"] = None

        if content_paragraphs:
            parsed_data["content"] = " ".join(
